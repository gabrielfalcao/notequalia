...
change_password_json = api.model(
    "ChangeUserPassword",
    {
        ...
        "confirm_new_password": PasswordField(
            required=True, description="confirm strong password"
        ),
    },
)
...
parser_retrieve_by_email.add_argument(
    "email", type=inputs.email(check=True), required=True, location="args"
)

# parser.add_argument('session', location='cookies', help='the session id containing the state of authentication')

user_ns = api.namespace("User API V1", description="Users API", path="/api/v1/users")
