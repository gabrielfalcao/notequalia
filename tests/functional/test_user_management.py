...
def test_create_user_ok(context):
    ...
    response = context.http.post(
        "/api/v1/users/",
        json={"email": "chuck@norris.com", "password": "I+:El_3366hLaH|Bh%Q'`"},
    )
...
def test_create_user_missing_pw(context):
    ...
    # Given that I perform a POST in /api/v1/user
    response = context.http.post("/api/v1/users/", json={"email": "chuck@norris.com"})
    ...
    response_errors_for_field(response, "password").should.contain(
...
def test_create_user_short_pw(context):
    ...
    # Given that I perform a POST in /api/v1/user
    response = context.http.post(
        "/api/v1/users/", json={"email": "chuck@norris.com", "password": "1234567"}
    )
...
def test_create_user_weak_pw(context):
    ...
    response = context.http.post(
        "/api/v1/users/",
        json={"email": "chuck@norris.com", "password": "0123456789abcdef"},
    )
    ...
    # And the response should have a meaningful message
    response_errors_for_field(response, "password").should.contain(
        "invalid password: should have at least one special character."
    )
...
def test_change_password_of_user(context):
    ...
    response = context.http.post(
        f"/api/v1/users/{created.id}/change-password",
        json={
            "email": "foo@bar.com",
